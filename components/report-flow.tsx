...
export function ReportFlow() {
  ...
  return (
    <div className="report-workspace">
      ...
      <nav className="report-progress" aria-label="Report progress">
        <ol className="ux4g-stepper ux4g-stepper-horizontal ux4g-stepper-s">
          {steps.map((label, index) => <li aria-current={index === draft.step ? 'step' : undefined} className={`ux4g-stepper-step ${index < draft.step ? 'ux4g-stepper-completed' : index === draft.step ? 'ux4g-stepper-inprogress' : 'ux4g-stepper-step-pending'}`} key={label}><span className="ux4g-stepper-head"><span className="ux4g-stepper-head-icon" aria-hidden="true">{index < draft.step ? <span className="ux4g-icon-outlined">check</span> : index + 1}</span><span className="ux4g-stepper-label">{label}</span></span></li>)}
        </ol>
      </nav>
      <div className="report-shell">
        ...
        {draft.step === 6 && <div><h2 id="active-step-title" className="ux4g-heading-l-strong">Review your complaint</h2><div className="review-sections"><section><h3>Incident summary</h3><p>{draft.narrative || 'Not provided'}</p><button className="ux4g-btn ux4g-btn-text-primary ux4g-btn-sm" onClick={() => update('step', 1)} type="button">Edit</button></section><section><h3>Transaction</h3><p>₹{draft.amount || 'Unknown'} · {draft.paymentMethod || 'Unknown'} · {draft.transactionId || 'No reference supplied'}</p><button className="ux4g-btn ux4g-btn-text-primary ux4g-btn-sm" onClick={() => update('step', 2)} type="button">Edit</button></section><section><h3>Suspect details</h3><p>{[draft.contactChannel, draft.suspectPhone, draft.suspectHandle, draft.suspectWebsite].filter(Boolean).join(' · ') || 'Unknown'}</p><button className="ux4g-btn ux4g-btn-text-primary ux4g-btn-sm" onClick={() => update('step', 3)} type="button">Edit</button></section><section><h3>Evidence</h3><p>{draft.evidence.length ? `${draft.evidence.length} attachment(s)` : 'No evidence attached'}</p><button className="ux4g-btn ux4g-btn-text-primary ux4g-btn-sm" onClick={() => update('step', 4)} type="button">Edit</button></section><section><h3>Citizen details</h3><p>{draft.fullName || 'Missing'} · {draft.mobile || 'Missing'} · {draft.state || 'State not provided'}</p><button className="ux4g-btn ux4g-btn-text-primary ux4g-btn-sm" onClick={() => update('step', 5)} type="button">Edit</button></section></div><label className="ux4g-checkbox ux4g-checkbox-md"><input className="ux4g-checkbox-input" type="checkbox" checked={draft.reviewed} onChange={(event) => update('reviewed', event.target.checked)} /><span className="ux4g-checkbox-control" aria-hidden="true"><span className="ux4g-checkmark" /></span><span className="ux4g-checkbox-label">I reviewed the information and corrected anything inaccurate.</span></label><label className="ux4g-checkbox ux4g-checkbox-md"><input className="ux4g-checkbox-input" type="checkbox" checked={draft.prototypeConsent} onChange={(event) => update('prototypeConsent', event.target.checked)} /><span className="ux4g-checkbox-control" aria-hidden="true"><span className="ux4g-checkmark" /></span><span className="ux4g-checkbox-label">I consent to save this report on this device and view its acknowledgement summary.</span></label><button className="ux4g-btn ux4g-btn-primary ux4g-btn-md" onClick={submit} type="button">Save report and view acknowledgement</button></div>}

          <div className="step-actions"><button className="ux4g-btn ux4g-btn-outline-primary ux4g-btn-md" disabled={draft.step === 0} onClick={() => update('step', Math.max(0, draft.step - 1))} type="button"><span className="ux4g-icon-outlined" aria-hidden="true">arrow_back</span> Back</button>{draft.step < 6 && draft.step !== 1 && <button className="ux4g-btn ux4g-btn-primary ux4g-btn-md" onClick={goNext} type="button">Continue <span className="ux4g-icon-outlined" aria-hidden="true">arrow_forward</span></button>}<a className="ux4g-btn ux4g-btn-text-primary ux4g-btn-md" href="/">Continue later</a></div>
        </section>
        <aside className="report-support" aria-label="Immediate safety guidance">
          <h2>First, reduce further loss</h2>
          <ul className="report-support-list">
            <li><a href="tel:1930"><span className="support-action-icon support-action-danger ux4g-icon-outlined" aria-hidden="true">call</span><span><strong>Call 1930</strong><small>Report immediately if money has been transferred.</small></span><span className="ux4g-icon-outlined" aria-hidden="true">chevron_right</span></a></li>
            <li><a href="/help"><span className="support-action-icon ux4g-icon-outlined" aria-hidden="true">account_balance</span><span><strong>Contact your bank</strong><small>Ask about stopping or recalling the transaction.</small></span><span className="ux4g-icon-outlined" aria-hidden="true">chevron_right</span></a></li>
            <li><a href="/help"><span className="support-action-icon support-action-warning ux4g-icon-outlined" aria-hidden="true">block</span><span><strong>Do not pay again</strong><small>Ignore requests for recovery fees or verification payments.</small></span><span className="ux4g-icon-outlined" aria-hidden="true">chevron_right</span></a></li>
            <li><a href="/learn"><span className="support-action-icon ux4g-icon-outlined" aria-hidden="true">save</span><span><strong>Preserve evidence</strong><small>Keep original messages, receipts and screenshots.</small></span><span className="ux4g-icon-outlined" aria-hidden="true">chevron_right</span></a></li>
          </ul>
          <div className="report-note ux4g-alert ux4g-alert-info ux4g-alert-wide" role="note"><span className="ux4g-alert-icon ux4g-icon-outlined" aria-hidden="true">info</span><div className="ux4g-alert-content"><h3 className="ux4g-alert-title">Why we ask this</h3><p className="ux4g-alert-message">These details help organise the report and guide you to the right next step.</p></div></div>
          <div className="report-note ux4g-alert ux4g-alert-warning ux4g-alert-wide" role="note"><span className="ux4g-alert-icon ux4g-icon-outlined" aria-hidden="true">privacy_tip</span><div className="ux4g-alert-content"><h3 className="ux4g-alert-title">Using a shared device?</h3><p className="ux4g-alert-message">Clear this draft and sign out when you finish.</p></div></div>
        </aside>
      </div>
    </div>
  );
